...
if (isset($_GET['actualizar'])) {
    ...
    $query = "UPDATE config SET usuario='$usuario', password='$password', totalPreguntas='$totalPreguntas' WHERE usuario='{$_SESSION['usuarioLogeado']}'";

    //actualizamos en la tabla configuracion
    if (mysqli_query($conn, $query)) { //Se actualizo correctamente
        $_SESSION['usuarioLogeado'] = $usuario; // Actualizamos el usuario logueado en la sesión
        $_SESSION['passwordlogeado'] = $password; // Actualizamos el password en la sesión
                ...
                <section id="configuracion">
                    <form action="<?php echo $_SERVER['PHP_SELF'] ?>" method="get">
                        <div class="fila">
                            ...
                            <input type="text" name="usuario" id="" value = "<?php echo htmlspecialchars($_SESSION['usuarioLogeado']); ?>" required>
                        </div>
                        <div class="fila">
                            <label for="">Password:</label>
                            <input type="text" name="password" id="" value = "<?php echo htmlspecialchars($_SESSION['passwordlogeado']); ?>" required>
                        </div>
                        <div class="fila">
                            <label for="">Total Preguntas por Juego</label>
                            <input type="number" name="totalPreguntas" id="" value = "<?php echo htmlspecialchars($config['totalPreguntas']); ?>" required>
                        </div>
                        <hr>
